            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_txt:  # Write to file
                        xyxy = (torch.tensor(xyxy).view(1, 4)).view(-1).numpy().astype(int).tolist()
                        if int(cls) == 7:  # TODO: 7为其他类别
                            continue
                        if cls in thresholds and conf < thresholds[cls]:
                            continue
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str,
                        default='/mnt/YuHe/data/val_data/huzhou/valid/3',
                        help='file/dir/URL/glob, 0 for webcam 需要预测文件夹的路径')
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int,
                        default=[960], help='inference size h,w')
    ...
    parser.add_argument('--project', default='/mnt/YuHe/data/val_data/huzhou/valid/ori_predict',
                        help='save results to project/name')
    parser.add_argument('--name', default='predict_txt', help='save results to project/name')
...
if __name__ == "__main__":
    opt_ = parse_opt()
    print(colorstr('detect: ') + ', '.join(f'{k}={v}' for k, v in vars(opt_).items()))
