...
# 自定义文件
from components import torch_utils, google_utils
from components.datasets import *
from components.utils import *
from models.yolo import Model
from Detection.yolov5 import test  # import test.py to get mAP after each epoch

mixed_precision = True  # 混合精度，加速计算
try:
...
# 如果本地存在超参数 txt 文件，则重新覆写
f = glob.glob('hyp*.txt')  # Overwrite hyp with hyp*.txt (optional)
if f:
    print('Using %s' % f[0])
    for k, v in zip(hyp.keys(), np.loadtxt(f[0])):
        hyp[k] = v
if hyp['fl_gamma']:  # Print focal loss if gamma > 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='yolov5s.yaml', help='*.cfg path 模型结构配置文件')
    parser.add_argument('--data', type=str,
                        default=r'elephant.yaml', help='*.data path 数据集信息')
