...
from Detection.yolov5.components.datasets import *
from Detection.yolov5.components.utils import *
from components import torch_utils, google_utils


def test(data, weights=None, batch_size=16, imgsz=640, conf_thres=0.001, iou_thres=0.6,
         save_json=False, single_cls=False, augment=False, model=None, dataloader=None,
         fast=False, verbose=False):  # 0 fast, 1 accurate
    if model is None:  # Initialize/load model and set device
    ...
    niou = iouv.numel()  # iouv = iouv[0].view(1)  # comment for mAP@0.5:0.95
