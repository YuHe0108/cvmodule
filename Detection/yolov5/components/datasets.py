...
class LoadImages:
    ...
    def __init__(self, path, img_size=416):
        ...
        self.video_flag = [False] * nI + [True] * nV  # 当前检测文件是否是图像
        self.mode = 'images'
        if any(videos):
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:  # 读取的文件是视频
            self.mode = 'video'
        ...
        # Padded resize
        img = letterbox(img0, new_shape=self.img_size)[0]  # 对图像进行 resize
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        img = np.ascontiguousarray(img)
        # cv.imwrite(path + '.letterbox.jpg', 255 * img.transpose((1, 2, 0))[:, :, ::-1])  # save letterbox image
...
class LoadWebcam:
    ...
    def __init__(self, pipe=0, img_size=416):
        ...
        self.pipe = pipe
        self.cap = cv.VideoCapture(pipe)  # video capture object
    ...
    def __next__(self):
        ...
        if self.pipe == 0:  # local camera
            ret_val, img0 = self.cap.read()
            img0 = cv.flip(img0, 1)  # flip left-right
        else:  # IP camera
            n = 0
            while True:
                n += 1
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
                 path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False,
        ...
        try:
            ...
            print('Exception: ', e, 'LoadImagesAndLabels')
            raise Exception('Error loading data from %s. See %s' % (path, help_url))

        # 图像按照 batch_size 设置信息
        ...
        if self.rect:  # Read image shapes (wh)
            ...
            if l.shape[0]:
                ...
                if extract_bounding_boxes:
                    ...
                    for j, x in enumerate(l):
                        ...
                        assert cv.imwrite(f, img[b[1]:b[3], b[0]:b[2]]), 'Failure extracting classifier boxes'
            else:
...
def random_affine(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, border=0):
    ...
    if (border != 0) or (M != np.eye(3)).any():  # image changed
        img = cv.warpAffine(img, M[:2], dsize=(width, height), flags=cv.INTER_LINEAR, borderValue=(114, 114, 114))
...
def reduce_img_size(path='../data/sm4/images', img_size=1024):  # from utils.datasets import *; reduce_img_size()
    ...
    for f in tqdm(glob.glob('%s/*.*' % path)):
        try:
            ...
            if r < 1.0:
                img = cv.resize(img, (int(w * r), int(h * r)), interpolation=cv.INTER_AREA)  # _LINEAR fastest
    ...
    for path in ['../data/sm4/images', '../data/sm4/background']:
        ...
        for ext in formats:  # ['.bmp', '.jpg', '.jpeg', '.png', '.tif', '.dng']
            for f in tqdm(glob.glob('%s/*%s' % (path, ext)), desc='Converting %s' % ext):
                cv.imwrite(f.replace(ext.lower(), '.bmp').replace(path, path + 'bmp'), cv.imread(f))
    ...
    for a, b, files in os.walk(dataset):
        for file in tqdm(files, desc=a):
            ...
            if s == '.txt':  # replace text
                ...
                cv.imwrite(p.replace(s, '.bmp'), cv.imread(p))
                if s != '.bmp':
                    os.system("rm '%s'" % p)
