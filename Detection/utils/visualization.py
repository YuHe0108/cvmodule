...
from Detection.utils import standard_fields as fields
...
def draw_text_in_image(img, text, pos, color, line_width):
    ...
    font_scale = 1
    line_type = 1
    bottom_left_corner_text = pos
    font = cv.FONT_HERSHEY_PLAIN
    cv.putText(img, text, bottom_left_corner_text, font, font_scale, color, line_type)
    text_width, _ = cv.getTextSize(text, font, font_scale, line_type)[0]
                ...
                if not skip_labels:
                    if not agnostic_mode:
                        if classes[i] in category_index.keys():
                            ...
                            class_name = category_index[classes[i]]  # 预测对象的名字
                        else:
                            class_name = 'N/A'
