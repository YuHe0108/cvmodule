...
if __name__ == '__main__':
    image = cv.imread(
        r'D:\Users\YingYing\Desktop\irregular\no_skull\4\SkyViewPACS_Image[1.3.46.670589.33.1.63705344642800388500001.5413348312460096225.11].jpg')
    snake = Snake(image, closed=True)

    # Window, window name and trackbars
    snake_window_name = "Snakes"
    controls_window_name = "Controls"
    cv.namedWindow(snake_window_name)
    cv.namedWindow(controls_window_name)
    cv.createTrackbar("Alpha", controls_window_name, math.floor(snake.alpha * 100), 100, snake.set_alpha)
    cv.createTrackbar("Beta", controls_window_name, math.floor(snake.beta * 100), 100, snake.set_beta)
    cv.createTrackbar("Delta", controls_window_name, math.floor(snake.delta * 100), 100, snake.set_delta)
    cv.createTrackbar("W Line", controls_window_name, math.floor(snake.w_line * 100), 100, snake.set_w_line)
    cv.createTrackbar("W Edge", controls_window_name, math.floor(snake.w_edge * 100), 100, snake.set_w_edge)
    cv.createTrackbar("W Term", controls_window_name, math.floor(snake.w_term * 100), 100, snake.set_w_term)
