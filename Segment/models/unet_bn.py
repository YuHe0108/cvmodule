...
# --------------------------------------构建UNet模型--------------------------------------
def unet_model(input_shape, num_classes, dims=32, relu=True):
    inputs = keras.Input(shape=input_shape)
    # inputs_1 = conv_block(inputs, dims, kernel_size=1, relu=relu)
    # 编码层， encoder0 经过卷积但是图像尺寸没变， encoder1缩小一半
    # encoder0_pool, encoder0 = encoder_block(inputs_1, dims, relu=relu)  # 1
    encoder1_pool, encoder1 = encoder_block(inputs, dims, relu=relu)  # 1/2
    encoder2_pool, encoder2 = encoder_block(encoder1_pool, dims * 2, relu=relu)  # 1/4
    encoder3_pool, encoder3 = encoder_block(encoder2_pool, dims * 4, relu=relu)  # 1/8
    encoder4_pool, encoder4 = encoder_block(encoder3_pool, dims * 8, relu=relu)  # 1/16

    # 中间层
    center = encoder_block(encoder4_pool, dims * 16, pool=False)  # 原始图像的1/16

    # 解码层
    decoder4 = decoder_block(center, encoder4, dims * 8, relu=relu)
    decoder3 = decoder_block(decoder4, encoder3, dims * 4, relu=relu)
    decoder2 = decoder_block(decoder3, encoder2, dims * 2, relu=relu)
    decoder1 = decoder_block(decoder2, encoder1, dims * 1, relu=relu)
...
if __name__ == '__main__':
    unet = unet_model(input_shape=(256, 256, 1), num_classes=1, dims=64)
    unet.summary()
