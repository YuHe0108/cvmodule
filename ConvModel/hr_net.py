...
def fuse_layer_1(x):
    ...
    x0_1 = layers.UpSampling2D(interpolation='bilinear')(x0_1)
...
def fuse_layer_2(x):
    ...
    x0_1 = conv_bn_layer(x[1], filters=32, kernel_size=1)
    x0_1 = layers.UpSampling2D(size=(2, 2), interpolation='bilinear')(x0_1)
    x0_2 = conv_bn_layer(x[2], filters=32, kernel_size=1)
    x0_2 = layers.UpSampling2D(size=(4, 4), interpolation='bilinear')(x0_2)
    x0 = layers.Add()([x[0], x0_1, x0_2])

    # 将x[0]下采样， x[2]上采样， 与x[1]相加之后作为输出
    x1_0 = conv_bn_layer(x[0], filters=64, kernel_size=3, strides=2)
    x1_2 = conv_bn_layer(x[2], filters=64, kernel_size=1, strides=1)
    x1_2 = layers.UpSampling2D(size=(2, 2), interpolation='bilinear')(x1_2)
...
def fuse_layer_3(x):
    ...
    x0_3 = layers.UpSampling2D(size=(8, 8), interpolation='bilinear')(x0_3)

    output = layers.Concatenate()([x[0], x0_1, x0_2, x0_3])
    return output


def final_layer(x, classes):
    """最后的输出层"""
    x = layers.UpSampling2D(interpolation='bilinear')(x)
    x = conv_bn_layer(x, classes, kernel_size=1)
    x = layers.Softmax()(x)
    return x


def seg_hrnet(input_shape, classes):
